...
# Функция для получения метки по индексу
...
def generate_choice_keyboard(indices):
    ...
    keyboard.append([InlineKeyboardButton("Следующие услуги", callback_data="show_more")])
    print(keyboard)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if context.user_data.get('waiting_for_feedback'):
        ...
        top_indices = cosine_similarities.argsort()[-15:][::-1]  # Показать 15 элементов
        ...
        if cosine_similarities[top_indices[0]] < 0.4:
            ...
            context.user_data['user_query'] = ""
            response = get_labeled_response(top_indices[:5])
            message_parts = split_message(f"Найденные похожие записи:\n\n{response}")
            for part in message_parts:
                await update.message.reply_text(part)

            choice_keyboard = generate_choice_keyboard(top_indices[:5])
...
async def choice_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == 'show_more':
        ...
        # Если счетчик равен 3, просим переформулировать запрос
        if current_index >= 3:
            await query.edit_message_text("Показаны все результаты. Пожалуйста, переформулируйте запрос.")
            return
        ...
        # Определяем новые результаты для показа
        start = next_index * 5
        end = start + 5
        if start >= len(top_indices):
            await query.edit_message_text("Показаны все результаты. Пожалуйста, переформулируйте запрос.")
            return

        response = get_labeled_response(top_indices[start:end])
        message_parts = split_message(f"Новые найденные записи:\n\n{response}")

        # Обновляем сообщение с новыми результатами
        for part in message_parts:
            await query.edit_message_text(part)

        # Обновляем клавиатуру для следующих 5 элементов
        choice_keyboard = generate_choice_keyboard(top_indices[start:end])
        await query.edit_message_reply_markup(reply_markup=choice_keyboard)

    else:
        # Обработать выбор пользователя (если он выбрал один из предложенных ответов)
